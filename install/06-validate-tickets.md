...
1. *"Pin the validator to which `eltonio450/rondo` ref? (default: `v0.3` — the tag shipped with this brick; `main` for bleeding-edge)"*
...
- **Validate the host repo's workflow file against the Rondo action inputs.** Not shipped in v0.3 — `action.yml` is the contract and GitHub Actions already fails fast on unknown `with:` keys.
