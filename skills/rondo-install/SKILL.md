...
1. Read [`INSTALL.md`](../../INSTALL.md) at the repo root of the Rondo source. If this skill was installed into the host repo as a standalone file, fetch `INSTALL.md` directly from the upstream Rondo repo at the same pin the scheduler brick uses — `https://github.com/eltonio450/rondo/blob/v0.1/INSTALL.md` — so the install steps stay aligned with the action version. Rondo is not vendored into the host repo by the install.
